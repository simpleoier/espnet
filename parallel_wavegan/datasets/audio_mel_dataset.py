...
class AudioMelDataset(Dataset):
    ...
    def __init__(self,
        ...
        if self.use_local:
            assert len(audio_files) == len(local_files), \
                f"Number of audio and local files are different ({len(audio_files)} vs {len(local_files)})."
        if self.use_global:
            assert len(audio_files) == len(global_files), \
                f"Number of audio and global files are different ({len(audio_files)} vs {len(global_files)})."
    ...
    def __getitem__(self, idx):
        ...
        if self.use_local:
            l = self.local_load_fn(self.local_files[idx])
            items = items + (l,)
    ...
    def __init__(self,
        ...
        if self.use_local:
            assert len(mel_files) == len(local_files), \
                f"Number of audio and local files are different ({len(mel_files)} vs {len(local_files)})."
        if self.use_global:
            assert len(mel_files) == len(global_files), \
                f"Number of audio and global files are different ({len(mel_files)} vs {len(global_files)})."
