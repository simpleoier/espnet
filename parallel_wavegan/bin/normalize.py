    ...
    for items in tqdm(dataset):
        ...
        if config["format"] == "hdf5":
            write_hdf5(os.path.join(args.dumpdir, f"{utt_id}.h5"),
                       args.target_feats, mel_norm.astype(np.float32))
            if not args.skip_wav_copy:
                write_hdf5(os.path.join(args.dumpdir, f"{utt_id}.h5"),
            ...
            np.save(os.path.join(args.dumpdir, f"{utt_id}-{args.target_feats}.npy"),
                    mel_norm.astype(np.float32), allow_pickle=False)
            if not args.skip_wav_copy:
            ...
            if config.get("use_global_condition", False):
                np.save(os.path.join(args.dumpdir, f"{utt_id}-global.npy"),
                        g.reshape(-1), allow_pickle=False)
        else:
            raise ValueError("support only hdf5 or npy format.")
