...
def main():
    ...
    parser.add_argument("--target-feats", type=str, default="feats", choices=["feats", "local"],
                        help="target name to compute statistics.")
    parser.add_argument("--utt2spk", default=None, type=str,
                        help="kaldi-style spk2utt file. if given, calculate "
                             "statistics of each speaker.")
    ...
    if args.utt2spk is None:
        ...
        for spk, scaler in spk2scaler.items():
            if config["format"] == "hdf5":
                write_hdf5(os.path.join(args.dumpdir, "stats.h5"), f"{spk}/mean", scaler.mean_.astype(np.float32))
                write_hdf5(os.path.join(args.dumpdir, "stats.h5"), f"{spk}/scale", scaler.scale_.astype(np.float32))
            else:
                stats = np.stack([scaler.mean_, scaler.scale_], axis=0)
                np.save(os.path.join(args.dumpdir, f"stats-{spk}.npy"), stats.astype(np.float32), allow_pickle=False)
