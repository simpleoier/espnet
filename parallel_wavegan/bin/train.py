...
class Trainer(object):
    ...
    def _train_step(self, batch):
        ...
        if self.config.get("use_subband_stft_loss", False):
            ...
            if not self.is_vq:
                y_mb = self.criterion["pqmf"].analysis(y)
            sub_sc_loss, sub_mag_loss = self.criterion["sub_stft"](
                y_mb_.view(-1, y_mb_.size(2)),  # (B, C, T) -> (B x C, T),
                y_mb.view(-1, y_mb.size(2)),  # (B, C, T) -> (B x C, T)
            )
...
class Collater(object):
    ...
    def __call__(self, batch):
        ...
        if self.use_aux_input:
            ...
            if self.use_local_condition:
                # check length
                batch = [self._adjust_length(b[0], b[1]) for b in batch if len(b[1]) >= self.mel_threshold]

                # decide random index
                l_lengths = [len(b[1]) for b in batch]
                l_starts = np.array([np.random.randint(
                    self.start_offset, ll + self.end_offset) for ll in l_lengths])
                l_ends = l_starts + self.batch_max_frames
                y_starts = l_starts * self.hop_size
                y_ends = y_starts + self.batch_max_steps

                # make random batch
                y_batch = [b[0][start: end] for b, start, end in zip(batch, y_starts, y_ends)]
                l_batch = [b[1][start: end] for b, start, end in zip(batch, l_starts, l_ends)]
                if self.use_global_condition:
                    g_batch = [b[2].reshape(1) for b in batch]
            else:
                # check length
                if self.use_global_condition:
                ...
                y_starts = np.array([np.random.randint(
                    self.start_offset, yl + self.end_offset) for yl in y_lengths])
                y_ends = y_starts + self.batch_max_steps
            ...
            y_batch = torch.tensor(y_batch, dtype=torch.float).unsqueeze(1)  # (B, 1, T)
            if self.use_local_condition:
                l_batch = torch.tensor(l_batch, dtype=torch.float).transpose(2, 1)  # (B, C' T')
            else:
                l_batch = None
            if self.use_global_condition:
...
def main():
    ...
    use_noise_input = "ParallelWaveGAN" in generator_type and "VQVAE" not in generator_type
