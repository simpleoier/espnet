...
def main():
    ...
    for utt_id, (audio, fs) in tqdm(dataset):
        ...
        # extract f0 sequence
        if args.extract_f0:
            l = logf0_and_vuv(audio, fs, config["hop_size"])
            if l is None:
                continue
            l = np.concatenate(l, axis=-1)
            if len(audio) > len(l) * config["hop_size"]:
                audio = audio[:len(l) * config["hop_size"]]
            if len(audio) < len(l) * config["hop_size"]:
                audio = np.pad(audio, (0, len(l) * config["hop_size"] - len(audio)), mode="edge")
        ...
        if config["format"] == "hdf5":
            ...
            if args.extract_f0:
                write_hdf5(os.path.join(args.dumpdir, f"{utt_id}.h5"), "local", l.astype(np.float32))
            ...
            if args.extract_f0:
                np.save(os.path.join(args.dumpdir, f"{utt_id}-local.npy"),
                        l.astype(np.float32), allow_pickle=False)
        else:
        ...
        if config.get("use_global_condition", False):
            ...
            idx = spk2idx[spk]
            if config["format"] == "hdf5":
                write_hdf5(os.path.join(args.dumpdir, f"{utt_id}.h5"), "global", int(idx))
            elif config["format"] == "npy":
                np.save(os.path.join(args.dumpdir, f"{utt_id}-global.npy"),
                        int(idx), allow_pickle=False)
