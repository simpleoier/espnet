...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 9 ] && [ ${stop_stage} -ge 9 ]; then
        ...
        # shellcheck disable=SC2086
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m espnet2.bin.vq_asr_train \
                --collect_stats true \
                --use_preprocessor true \
                --bpemodel "${bpemodel}" \
                --token_type "${token_type}" \
                ...
                --cleaner "${cleaner}" \
                --g2p "${g2p}" \
                --train_data_path_and_name_and_type "${_asr_train_dir}/text_spk1,text_spk1,text" \
                --train_data_path_and_name_and_type "${_asr_train_dir}/text_spk2,text_spk2,text" \
                --train_data_path_and_name_and_type "${_asr_train_dir}/vq_spk1,vq_spk1,text" \
                --train_data_path_and_name_and_type "${_asr_train_dir}/vq_spk2,vq_spk2,text" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/text_spk1,text_spk1,text" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/text_spk2,text_spk2,text" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/vq_spk1,vq_spk1,text" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/vq_spk2,vq_spk2,text" \
                --train_shape_file "${_logdir}/train.JOB.scp" \
    ...
    if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
        ...
        if [ "${num_splits_asr}" -gt 1 ]; then
            ...
            _opts+="--train_data_path_and_name_and_type ${_split_dir}/text_spk1,text,text "
            _opts+="--train_shape_file ${_split_dir}/text_shape.${token_type} "
            _opts+="--multiple_iterator true "

        else
            _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/vq_spk1,vq_spk1,text "
            _opts+="--train_shape_file ${asr_stats_dir}/train/vq_spk1_shape "
            _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/vq_spk2,vq_spk2,text "
            _opts+="--train_shape_file ${asr_stats_dir}/train/vq_spk2_shape "
            _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/text_spk1,text_spk1,text "
            _opts+="--train_shape_file ${asr_stats_dir}/train/text_spk1_shape "
            _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/text_spk2,text_spk2,text "
            _opts+="--train_shape_file ${asr_stats_dir}/train/text_spk2_shape "
        ...
        ${python} -m espnet2.bin.launch \
            ...
            ${python} -m espnet2.bin.vq_asr_train \
                ...
                --g2p "${g2p}" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/vq_spk1,vq_spk1,text" \
                --valid_shape_file "${asr_stats_dir}/valid/vq_spk1_shape" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/vq_spk2,vq_spk2,text" \
                --valid_shape_file "${asr_stats_dir}/valid/vq_spk2_shape" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/text_spk1,text_spk1,text" \
                --valid_shape_file "${asr_stats_dir}/valid/text_spk1_shape" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/text_spk2,text_spk2,text" \
                --valid_shape_file "${asr_stats_dir}/valid/text_spk1_shape" \
                --resume true \
                ${_fold_length_param} \
    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        for dset in ${test_sets} ${valid_set}; do
            ...
            ${_cmd} --gpu "${_ngpu}" JOB=1:"${_nj}" "${_logdir}"/asr_inference.JOB.log \
                ${python} -m espnet2.bin.vq_asr_inference \
                    --ngpu "${_ngpu}" \
                    --data_path_and_name_and_type "${_data}/vq_spk1,vq_spk1,text" \
                    --data_path_and_name_and_type "${_data}/vq_spk2,vq_spk2,text" \
            ...
            for f in token token_int score text; do
                for i in $(seq "${_nj}"); do
                    cat "${_logdir}/output.${i}/1best_recog_spk1/${f}"
                done | LC_ALL=C sort -k1 >"${_dir}/${f}_spk1"

                for i in $(seq "${_nj}"); do
                    cat "${_logdir}/output.${i}/1best_recog_spk2/${f}"
                done | LC_ALL=C sort -k1 >"${_dir}/${f}_spk2"
            done
        done
    fi
            ...
            for _type in cer wer ter; do
                ...
                for spk in $(seq "2"); do
                    ...
                    paste \
                        ...
                        <(<"${_data}/text_spk${spk}" awk -v spkid=spk${spk} '{ print "(" $1 "-" spkid ")" }') \
                            >>"${_scoredir}/ref.trn"
