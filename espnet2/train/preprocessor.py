...
class PhonemePreprocessor_multi(AbsPreprocessor):
    def __init__(
        self,
        train: bool,
        token_type: str = None,
        ...
        if token_type is not None:
            ...
            self.token_id_converter = PhoneIDConverter(
                token_list=token_list,
            )
        else:
            self.text_cleaner = None
    ...
    def __call__(
        ...
        if "speakers_str" in data:
            ...
            spk_to_idx_list = list(spk_to_idx.values())
            if data["speakers_str"] in spk_to_idx:
                data["spk_str"] = np.array([spk_to_idx[data["speakers_str"]]], dtype=np.int64)
            else:
                data["spk_str"] = np.array([random.choice(spk_to_idx_list)], dtype=np.int64)
