...
class RNNEncoderMix_spk(AbsEncoder):
    ...
    def forward(
        ...
        xs_pad_sd_spks = [xs_pad for i in range(self.num_spkrs)]
        ilens_sd = [ilens for i in range(self.num_spkrs)]
        ilens_sd_spks = [ilens for i in range(self.num_spkrs)]
        for ns in range(self.num_spkrs):
            # Encoder_SD: speaker differentiate encoder
            # print('xs_pad_spks:', xs_pad_sd_spks[ns].shape)
            ...
            for module in self.enc_rec:
                xs_pad_sd[ns], ilens_sd[ns], _ = module(xs_pad_sd[ns], ilens_sd[ns])
            # print('xs_pad:', xs_pad_sd[ns][0,100:110])
        ...
        return xs_pad_sd, ilens_sd, xs_pad_sd_spks, ilens_sd_spks
