...
from espnet2.tasks.vq_asr import ASRTask
...
class Speech2Text:
    ...
    @torch.no_grad()
    def __call__(
        self, vq_spk1: Union[torch.Tensor, np.ndarray], vq_spk2:Union[torch.Tensor, np.ndarray]
        ...
        if isinstance(vq_spk1, np.ndarray):
            vq_spk1 = torch.tensor(vq_spk1)
            vq_spk2 = torch.tensor(vq_spk2)
        
        # data: (Nsamples,) -> (1, Nsamples)
        # speech = speech.unsqueeze(0).to(getattr(torch, self.dtype))

        assert vq_spk1.dim() == vq_spk2.dim() ==1, (vq_spk1.shape, vq_spk2.shape)
        vq_spk1 = vq_spk1.unsqueeze(0).long()
        vq_spk2 = vq_spk2.unsqueeze(0).long()
        ...
        for spk_idx in range(2):
            nbest_hyps = self.beam_search(
                x=enc[spk_idx], maxlenratio=self.maxlenratio, minlenratio=self.minlenratio
            )
    ...
    with DatadirWriter(output_dir) as writer:
        for keys, batch in loader:
            ...
            try:
                ...
                results_list = [[[" ", ["<space>"], [2], hyp]] * nbest]
